...
ares <- atime::atime(
  ...
  logcondens = {
    ...
    data.table(
      solution = list(logcondens::isoMean(target - l_vec, weights) + l_vec)
    )
  },
  quadprog = {
    k <- length(target)
...
][, ok := mapply(function(i, q) isTRUE(all.equal(i, q)), logcondens, quadprog)]
