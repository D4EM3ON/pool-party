# Taken from commit d0ea08d5f3cdc49a17a3d4fdae498876695326eb:
# https://github.com/r-devel/r-svn/blob/d0ea08d5f3cdc49a17a3d4fdae498876695326eb/src/library/stats/R/isoreg.R
...
## `C_isoreg` is a native symbol object created by stats' NAMESPACE
## (`useDynLib(stats, ..., .registration = TRUE)`); it lives only inside that
## namespace, so sourcing this file on its own leaves it unbound -- hence
## "object 'C_isoreg' not found".
##
## `stats:::C_isoreg` would fix the error, but it would run *today's* C code
## behind the 2021 R wrapper. We want the 2021 routine, so we compile the
## isoreg.c pinned next to this file (same commit as the R code above) and bind
## C_isoreg to that. The shared object is built once per R session, in tempdir().
C_isoreg <- local({
  here <- local({
    ofile <- NULL
    for (i in seq_len(sys.nframe())) {
      of <- sys.frame(i)$ofile
      if (!is.null(of)) ofile <- of
    }
    if (!is.null(ofile)) {
  ...
  build <- file.path(tempdir(), "isoreg2021")
  dir.create(build, showWarnings = FALSE)
  src <- file.path(build, "isoreg2021.c")
  dll <- sub("[.]c$", .Platform$dynlib.ext, src)
  if (!file.exists(dll)) {
    file.copy(file.path(here, "isoreg.c"), src, overwrite = TRUE)
    status <- system2(
      file.path(R.home("bin"), "R"),
      c("CMD", "SHLIB", shQuote(src)),
      stdout = FALSE, stderr = FALSE
    )
    if (status != 0L || !file.exists(dll)) {
      stop("could not compile ", src, " (R CMD SHLIB failed)")
    }
  }
  if (!"isoreg2021" %in% names(getLoadedDLLs())) dyn.load(dll)
  getNativeSymbolInfo("isoreg", PACKAGE = "isoreg2021")
})

### Isotonic Regression --- original code is simplification of MASS' Shepard():
##
isoreg_old <- function(x, y = NULL) {
  xy <- xy.coords(x, y)
  x <- xy$x
  ...
  structure(
    c(
      xy[c("x", "y")], z[c("yf", "yc", "iKnots")],
      list(
        isOrd = isOrd, ord = if (!isOrd) ord,
        call = match.call()
      )
    ),
    class = "isoreg"
...
plot.isoreg <-
  function(x, plot.type = c("single", "row.wise", "col.wise"),
    ...
    if (both) {
      ...
      op <- par(
        mfcol = if (col.wise) 1L:2 else 2:1,
    ...
    points(xx[x$iKnots], x$yf[x$iKnots],
      col = par.fit$col,
    ...
    if (both) { ## Cumulative Plot
      ...
      points(x0[i], cy[i],
        col = par.fit$col, cex = par.fit$cex,
        pch = par.fit$pch
      )
      if (grid) {
        Agrid <- formals("grid")
        abline(
          v = x0[i], col = Agrid$col, lty = Agrid$lty,
          xpd = !col.wise
        )
      }
      ...
      if (!is.null(main)) {
        mtext(main,
          side = 3, outer = TRUE, cex = par("cex.main"),
          col = par("col.main"), font = par("font.main")
        )
      }
    }
